...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  async function fetchFeaturedProjects() {
    try {
      const response = await fetch('/pr-preview/pr-17/src/data/projects/featured-projects.json');
  ...
  async function fetchAllProjects() {
    try {
      const response = await fetch('/pr-preview/pr-17/src/data/projects/projects.json');
    ...
    sortedProjects.forEach(project => {
      ...
      projectCard.innerHTML = `
        <div class="project-image">
          <img
            src="/pr-preview/pr-17/assets/images/projects/${defaultImage}"
            alt="${project.title}"
            class="default-img"
            loading="lazy"
          />
          <img
            src="/pr-preview/pr-17/assets/images/projects/${hoverImage}"
