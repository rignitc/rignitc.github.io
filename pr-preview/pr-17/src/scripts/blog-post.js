...
async function checkCoverImage(slug) {
  try {
    const resp = await fetch(`/pr-preview/pr-17/content/${slug}/imgs/cover.webp`, { method: "HEAD" });
    if (resp.ok) {
      return `/pr-preview/pr-17/content/${slug}/imgs/cover.webp`;
    }
  } catch (err) {
    // Ignore errors
  }
...
async function loadMarkdownContent(slug) {
  try {
    const resp = await fetch(`/pr-preview/pr-17/content/${slug}/index.md`, { cache: "no-cache" });
...
function processImagePaths(markdown, slug) {
  return markdown.replace(
    ...
    (match, alt, src) => {
      ...
      return `![${alt}](/pr-preview/pr-17/content/${slug}/${src})`;
    }
  );
}
...
async function renderPost() {
  ...
  if (!slug) {
    document.getElementById("blog-post-content").innerHTML =
      '<div class="loading">Post not found. <a href="/pr-preview/pr-17/blog/">Return to blog</a></div>';
    return;
  }
  ...
  if (!markdown) {
    document.getElementById("blog-post-content").innerHTML =
      '<div class="loading">Post content not found. <a href="/pr-preview/pr-17/blog/">Return to blog</a></div>';
    return;
  }
  ...
  const postHTML = `
    <div class="post-header">
      <a href="/pr-preview/pr-17/blog/" class="back-to-blog-btn">
        <span>←</span>
